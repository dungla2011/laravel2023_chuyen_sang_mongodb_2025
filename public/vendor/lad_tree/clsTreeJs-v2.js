...
//Nếu dữ liệu ajax thì api có option load tất cả data về
    ...
    addPadZoneTo(nodeId, cond) {
        ...
        var newDiv = clsTreeJsV2.htmlToElement('<div data-code-pos="ppp1695969972549" class="empty_node_pad" ' +
...
//Vùng nào ko còn Node, thì bỏ hết đệm
        ...
        if (this.api_data) {
            var jqXHR = $.ajax({
                ...
                success: function (result) {
                    ...
                    // return mRet  = result
                    let ajaxSuccessEvent = new CustomEvent("move_node_success", {
                        detail: {
                            message: result,
                            time: new Date(),
                        },
                ...
                error: function (result) {
                    ...
                    if(result.responseJSON && result.responseJSON.message)
                        showToastWarningTop("Có lỗi xảy ra 310:" + result.responseJSON.message,"", 10000)
                    else
                        alert("Có lỗi xảy ra 12!")
    ...
    getStrTreeNode(pid, dataIn) {
        ...
        //Nếu ko thấy PID, thì cứ show All lên:
        // if(!foundPid){
        //     for (let elm1 of dataIn) {
        //         // console.log("ELM " , elm1);
        //         strAll += this.getHtmlOneNodeNotEndDiv(elm1)
    ...
    showTree() {
        ...
        if(this.api_data.indexOf("|") > 0){
            this.api_param_ex = this.api_data.split("|")[1];
            this.api_data = this.api_data.split("|")[0];
        }
        ...
        if (this.opt_open_all_first)
            url += '&get_all=1';
        url += "&" + this.api_param_ex;


        if(!this.api_data){
            alert("Not found api data to show tree! may be need set api_data for model (getHardCodeMetaObj/objMeta->join_api, $objMetaFolder->getApiUrl)!")
            return;
        }
        ...
        if (this.api_data) {
            ...
            $.ajax({
                ...
                error: function (result) {
                    ...
                    if(result.responseJSON && result.responseJSON.message)
                        showToastWarningTop("Có lỗi xảy ra 31:" + result.responseJSON.message + " \n" + url,10000)
                    else
                        showToastWarningTop("Có lỗi xảy ra 21",10000)
    ...
    getHtmlOneNodeNotEndDiv(elm, full = null) {
        ...
        if(elm.id == this.root_id && this.hide_root_node){
            ...
            console.log(" Hide root node" , this.hide_root_node);
        }
        var padHideMenu = ''
        if(this.disable_menu){
            padHideMenu = ';display: none;'
            // console.log(" disable menu root node");
        }
        ...
        if(elm.id == this.root_id){
            ...
            console.log(" elm.id == this.root_id ");
        }
        ...
        let str = '<div data-code-pos="ppp1695970017332" title="Duyệt các con của: '+ elm.id +'" style="" class="real_node_item ' +
            padClassRoot0 + '" data-parent="'+ elm.parent_id +'" data-has-child="' + elm.has_child + '" data-tree-node-id="' + elm.id + '" ' +
            ...
            str += '<div data-code-pos="pp0394850345" style="display: inline-block; float: right; ' +
                padHideRoot + '" class="node_parent_list_click' + padClassRoot + '"> ' +
                '<i title="Duyệt cả con, cháu..."  class="fa fa-search"></i>  '

            if(elm._public_link){
                str += '<a title="Xem link public..."  href="'+ elm._public_link +'" target="_blank">' +
                    '<i style="margin-left: 10px; color: red" class="fa fa-link"></i></a> ';
            }
...
//        this.getNodeByDataId(elm.parent_id).prepend(clsTreeJsV2.htmlToElement(str));
        //real_node_item

        // Tìm phần tử cha
        let parentElement = this.getNodeByDataId(elm.parent_id);
        //Trường hợp browse 2 sẽ không có parrent
        if(!parentElement){
            console.log(" Not found parent so not add new node on UI?");
            return;
        }
        // Tìm phần tử con đầu tiên có class là 'real_node_item'
        let firstChild = parentElement.querySelector('.real_node_item');
        // Chèn element mới vào trước phần tử con đầu tiên
        if(firstChild)
            parentElement.insertBefore(clsTreeJsV2.htmlToElement(str), firstChild);
        else
            parentElement.append(clsTreeJsV2.htmlToElement(str));
...
//    this.toggleOpenNodeData(pid);
        ...
        if (this.api_data) {
            var jqXHR = $.ajax({
                ...
                success: function (result) {
                    ...
                    // return mRet  = result
                    that.tmp_cmd_add_result = 1
                    let ajaxSuccessEvent = new CustomEvent("add_node_success", {
                        detail: {
                            message: result,
                            time: new Date(),
                        },
        ...
        if(this.treeType == 'horizontal_folder'){
            return 1;
        }

        this.addNewNodeUi(elm)

        if (typeof addExtraInfoToNode === 'function') {
            addExtraInfoToNode()
        }
...
//Tìm tất cả các con, nếu chưa thấy con, thì kích hoạt mở ra:
    ...
    static getInstanceTreeOfAnyObject(obj) {
        ...
        console.log(obj, "Get instance ..." + bind_selector);
    ...
    openModalOfNode(nodeId, cmd, textDesc, textButton) {
        ...
        console.log("Create child in NODE1 = ", node);
        //Neu co node, thi hien thi dialog tai vi tri node
        if(node) {
        ...
        dialog.style.left = "" + (window.innerWidth / 2 - 150) + "px";
        dialog.style.top = "250" + "px";
...
$(function () {
    ...
    $(".modal_dialog_edit_node input.btn_create").on('click', function () {
        ...
        if (cmd == 'create_node') {
            ...
            if(tree1.treeType == 'horizontal_folder'){
            }
            else
    ...
    $.contextMenu({
        ...
        callback: function (key, options) {
            ...
            if (key == 'cut') {
                ...
                sessionStorage.setItem('cutting_id', thisNodeId);
            }
            ...
            if (key == 'paste') {
                ...
                if(!tree1.cutting_id)
                if(sessionStorage.getItem('cutting_id')){
                    tree1.cutting_id = sessionStorage.getItem('cutting_id');
                }

                sessionStorage.setItem('cutting_id',null);

                console.log(" Cut pate: " + tree1.cutting_id)
                if(tree1.cutting_id == 'null' || !tree1.cutting_id || tree1.cutting_id == null)
                    return;
               // console.log("Node of cutting "  , tree1.getNodeByDataId(tree1.cutting_id));

                //Gán parent cũ để sort lại sau khi đã cắt
                if(tree1.getNodeByDataId(tree1.cutting_id) && tree1.getNodeByDataId(tree1.cutting_id).parentElement)
                ...
                let moveTo = thisNodeId;
                if(moveTo == 0 && tree1.currentNodeToPaste != 0)
                    moveTo = tree1.currentNodeToPaste;

                console.log(" Movex to" , moveTo, tree1.currentNodeToPaste);

                if (tree1.moveNode(tree1.cutting_id, moveTo)) {
                    if(tree1.treeType == 'horizontal_folder'){
                        return;
                    }
        ...
        items: {
            ...
            "create_node": {name: "Create", icon: "edit", visible: function (key, options) {
