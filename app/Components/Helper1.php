...
class Helper1
{
    ...
    public static function getValidateStringAlt()
    {
        $messages = [
            'required' => 'Cần nhập: :attribute ',
            'string' => ':attribute : phải là dạng chuỗi',
            'file' => ':attribute : phải là file',
            'mimes' => 'Chỉ hỗ trợ dạng file :attribute / :mimes',
            'max' => ':attribute Tối đa ký tự :max',
            'min' => ':attribute tối thiểu ký tự :min',
            'unique' => ':attribute không được trùng lặp (đã được tạo trước đó?)',
            'digits_between' => ':attribute yêu cầu phải từ: :min đến :max số',
    ...
    public static function imageShow1($obj, $val, $field, $getData = 0)
    {
        ...
        foreach ($mIdFile as $idF) {
            ...
            if ($objFile = FileUpload::find($idF)) {
                ...
                if (strstr($objFile->mime, 'image') !== false ||
                strstr($objFile->mime, 'video') !== false||
                strstr($objFile->mime, 'pdf') !== false
                ) {
                    $thumb = $objFile->getCloudLinkImage();
                    $fileImg = "<img style='min-width: 60px; min-height: 40px; border: 1px solid green' src='$thumb' alt='' title='$objFile->name , $objFile->created_at'>";
                } else {
