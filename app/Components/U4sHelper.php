...
class U4sHelper
{
    ...
    function __construct($uid) {
        ...
        if(!$this->objUserCloud){
            loi("Not found cloud info!");
        }
        if(!$this->objUserCms){
            loi("Not found user info!");
        }
...
//                            $mSv = ['sv18.4share.vn', 'sv96a.4share.vn'];
        ...
        $str = '';
        $str .= "\n <span> Free : " . ByteSize($totalFreeDisk) . "/" . ByteSize($totalSizeDisk) . "</span>";
...
//        $urlCheck = "http://$server:" . SERVER_INFO_WEB_PORT . "/tool/sysinfo_glx.php?checkfile=$strHex";
        ...
        if (strstr($check, "OKFILETHIS:$locationFull") === FALSE)
            loi("Error: File đã bị xoá hoặc không hợp lệ! CHECK: $check");
