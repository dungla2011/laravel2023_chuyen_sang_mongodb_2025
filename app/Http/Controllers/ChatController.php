...
class ChatController extends Controller
{
    ...
    private function getUserConversations($userId, $channel_name = null)
    {
        ...
        $latestMessages = CrmMessage::select('thread_id', 'content', 'created_at', 'd_name', 'uid_from', 'id_to', 'type' , 'is_self')
...
//                $message->formatted_date = $message->ts ? $message->ts->format('d/m') : '';
                ...
                if($tmp = json_decode($message->content)){
                    if($tmp->thumb ?? '') {
                        ...
                        $message->content = '<a href="' . $tmp->thumb . '" target="_blank"><img src="' . $tmp->thumb . '" style="max-width: 600px; "/></a>';
