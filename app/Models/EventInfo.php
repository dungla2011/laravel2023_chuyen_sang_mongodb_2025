...
//            EventInfo::$DEF_QRCODE[0] => $ev->getQrCode(),
    ...
    static  function getStrTimeStartEnd($ev) {
        ...
        if($ev->time_start){
            ...
            if($ev->time_end) {
                ...
                $hourEnd = "-" . date('H:i', strtotime($ev->time_end));
            }

            if(date('d/m/Y', strtotime($ev->time_start)) == date('d/m/Y', strtotime($ev->time_end)))
                $strFullTIme = __('reg_event_ncbd.time_start').": $dateStart ". __('reg_event_ncbd.hour_start').": $hourStart $hourEnd " ;
            else{
                $hourEnd = trim($hourEnd, '-');
                $dateEnd = trim($dateEnd, '-');
                $strFullTIme = __('reg_event_ncbd.time_start')."  $dateStart ". __('reg_event_ncbd.hour_start')." $hourStart  - ".__('reg_event_ncbd.time_start')." $dateEnd ". __('reg_event_ncbd.hour_start')." $hourEnd " ;
                if(!$ev->time_end)
                    $strFullTIme = __('reg_event_ncbd.time_start')."  $dateStart - ". __('reg_event_ncbd.hour_start')." $hourStart  " ;
            }
        }
        return $strFullTIme;
    }
    ...
    static function getHtmlSelectEvent($linkOpt, $mmEv, $key)
    {
        echo "<select onchange='location.href=this.value || \"$linkOpt\"' class='form-control form-control-sm bg-light text-primary mb-3' style='color: royalblue!important; font-weight: bold'>";
