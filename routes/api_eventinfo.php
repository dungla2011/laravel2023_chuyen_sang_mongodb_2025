...
Route2::prefix('/event-info')->group(function () {
    ...
    $routeName = 'api.'.$nameModule.'.getUserListEvent';
    $r = Route2::get('/getUserListEvent', [
        \App\Http\ControllerApi\EventInfoControllerApi::class, 'getUserListEvent',
    ])->name($routeName);
    if ($r instanceof Route2);
    $r->middleware('can:'.$routeName);
    $r->route_group_desc_ = $route_group_desc;
    $r->route_desc_ = 'copyTemp user to event';
    $r->modelUsing_ = $modelUsing_;



    $routeName = 'api.'.$nameModule.'.copyTemp';
    $r = Route2::get('/copyTemp', [
        \App\Http\ControllerApi\EventInfoControllerApi::class, 'copyTemp',
    ])->name($routeName);
    if ($r instanceof Route2);
//    $r->middleware('can:'.$routeName);
    $r->route_group_desc_ = $route_group_desc;
    ...
    $routeName = 'api.'.$nameModule.'.addSubEventAndGetHtml';
    $r = Route2::post('/addSubEventAndGetHtml', [
        \App\Http\ControllerApi\EventInfoControllerApi::class, 'addSubEventAndGetHtml',
    ])->name($routeName);
    if ($r instanceof Route2);
    $r->middleware('can:'.$routeName);
    $r->route_group_desc_ = $route_group_desc;
    $r->route_desc_ = 'addSubEventAndGetHtml';
    $r->modelUsing_ = $modelUsing_;



    $routeName = 'api.'.$nameModule.'.removeSubEvent';
    $r = Route2::post('/removeSubEvent', [
        \App\Http\ControllerApi\EventInfoControllerApi::class, 'removeSubEvent',
    ])->name($routeName);
    if ($r instanceof Route2);
    $r->middleware('can:'.$routeName);
    $r->route_group_desc_ = $route_group_desc;
    $r->route_desc_ = 'removeSubEvent';
    $r->modelUsing_ = $modelUsing_;
